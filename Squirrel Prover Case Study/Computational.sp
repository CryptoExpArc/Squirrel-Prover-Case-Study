...
process clientAToCompromised(i,j,k:index) =
let pkA = pk(skA(i)) in
out(cA, pkA);
in(cA,x);
let DpkB = fst(x) in
let y = a_dec(x,skA(i)) in
let Dsk = fst( getSigMessage(y, DpkB) ) in
if ver(<Dsk, pkA>, y, DpkB) && DpkB = pk(skB(j)) then
  out(cA,s_enc(s(i), r(i), Dsk))
else
  out(cA, diff(ok, ko)).
 
system main = out(cA, pkA); out(sB, pkB);
             (  (!_i !_j !_k cA: clientA(i,j,k)) | (!_j !_k sB: serverB(j,k)) 
              | (!_i !_j !_k DcA: clientAToCompromised(i,j,k))                ). 
...
->
System Empty registered with actions (init).
System main registered with actions  (init,A,A1,cA1,cA2,cA3,sB1,sB2,DcA,DcA1,DcA2).
